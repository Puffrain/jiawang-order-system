...
export async function createSession(userId: string, role: "owner" | "buyer", request: Request, response: NextResponse) {
...
export async function currentSession() {
  ...
  const row = db.prepare(`SELECT s.id AS sessionId, s.user_id AS userId, u.phone, u.role, u.display_name AS displayName, u.tour_completed AS tourCompleted FROM auth_sessions s JOIN users u ON u.id = s.user_id WHERE s.token_hash = ? AND s.revoked_at IS NULL AND datetime(s.expires_at) > CURRENT_TIMESTAMP AND u.status = 'active'`).get(tokenHash(token)) as { sessionId: string; userId: string; phone: string; role: "owner" | "buyer"; displayName: string | null; tourCompleted: number } | undefined;
  return row ?? null;
}
