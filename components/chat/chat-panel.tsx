...
import ProductMessageCard, {
  RecommendedProduct,
} from "@/components/chat/product-message-card";
...
export default function ChatPanel({
    ...
    async (silent = false) => {
      ...
      try {
        ...
        const response = await fetch(
          `/api/chat/messages${params.size ? `?${params}` : ""}`,
          { cache: "no-store" },
        );
  ...
  return (
    <section className="flex h-full min-h-0 flex-col overflow-hidden rounded-2xl bg-white sm:rounded-3xl">
      <header className="flex items-center justify-between border-b p-4">
        <div className="flex min-w-0 items-center gap-2">
          <MessageCircle className="shrink-0 text-orange-500" />
        ...
        <button
          ...
          disabled={refreshing}
          onClick={() => load()}
          className="grid h-10 w-10 place-items-center rounded-lg bg-slate-100 disabled:opacity-50"
          aria-label="刷新消息"
        >
      ...
      <div className="min-h-0 flex-1 space-y-3 overflow-y-auto bg-[#f7f8fb] p-3 sm:p-4">
        {messages.map((message) => {
          ...
          const mediaId =
            ...
            (message.type === "image" ? message.id : undefined);
          ...
          return (
            ...
            >
              <div
                className={`max-w-[88%] rounded-2xl px-3 py-2 text-sm sm:max-w-[82%] ${message.type === "system" ? "w-full bg-orange-50 text-orange-900" : mine ? "bg-orange-500 text-white" : "bg-white text-slate-800 shadow-sm"}`}
              >
                {message.type === "voice" ? (
                  <VoiceMessage
                    id={message.id}
                    duration={message.payload?.durationSeconds}
                    mine={mine}
                  />
                ...
                {message.orderId && (
                  <OrderMessageCard
                    orderNo={message.payload?.orderNo || message.orderNo}
                    total={message.payload?.totalAmount ?? message.orderTotal}
                    status={
                      message.orderStatus
                        ? orderStatusLabel(message.orderStatus)
                        : undefined
                    }
                    kind={message.payload?.kind}
                    onClick={
                      onOpenOrder
                        ? () => onOpenOrder(message.orderId!)
                        : undefined
                    }
                  />
                )}
                <p
                  className={`mt-1 text-[10px] ${mine && message.type !== "system" ? "text-orange-100" : "text-slate-400"}`}
                >
      ...
      <form
        onSubmit={send}
        className="flex flex-wrap items-center gap-2 border-t p-3 pb-[max(.75rem,env(safe-area-inset-bottom))]"
      >
        <VoiceRecorder
        ...
        {canRecommendProducts && (
          <button
            ...
            className="grid h-11 w-11 shrink-0 place-items-center rounded-xl bg-slate-100 text-orange-600"
            title="推荐商品"
            aria-label="推荐商品"
          >
            <PackagePlus size={18} />
        ...
        <input
          ...
          maxLength={1000}
          placeholder="输入消息…"
          className="min-w-[10rem] flex-1 rounded-xl border border-slate-200 px-3 py-3 text-sm"
        />
        <button
          disabled={sending || !content.trim()}
          className="grid h-11 w-11 shrink-0 place-items-center rounded-xl bg-orange-500 text-white disabled:opacity-50"
          aria-label="发送文字消息"
        >
          <Send size={17} />
        </button>
      </form>
      {canRecommendProducts && showProductPicker && (
        <ProductPicker
          onPick={sendProduct}
