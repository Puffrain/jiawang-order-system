...
type Product = { id: string; name: string; category: string; brand?: string; description?: string; status?: "active" | "inactive"; archived?: boolean; permanentlyHidden?: boolean; primaryImage: { url: string } | null; images: { id: string; url: string }[]; skus: Sku[]; reviews?: { count:number; average:number; reviews:Array<{rating:number;content:string;buyerName:string;merchantReply?:string|null}> }; recommendations?: Product[]; relatedProducts?: Product[] };
...
export default function ProductDetail({ productId }: { productId: string }) {
  ...
  return <main className={`min-h-screen bg-[#f4f5f7] ${merchantPreview ? "pb-8" : "pb-28"} lg:px-6 lg:py-8`}><div className="mx-auto min-h-screen max-w-[520px] overflow-hidden bg-white lg:max-w-[1280px] lg:overflow-visible lg:rounded-lg lg:shadow-sm">
    <header className="sticky top-0 z-20 flex min-w-0 items-center gap-3 border-b bg-white/95 px-4 py-3 backdrop-blur lg:rounded-t-lg lg:px-8 lg:py-4"><Link href={back.href} aria-label={back.label} className="grid h-10 w-10 shrink-0 place-items-center rounded-full bg-slate-100"><ChevronLeft /></Link><b className="min-w-0 flex-1 truncate">{merchantPreview ? "商品预览" : "商品详情"}</b>{merchantPreview && <span className="shrink-0 rounded-full bg-sky-50 px-3 py-1 text-xs font-semibold text-sky-700">购买控件已隐藏</span>}</header>
    <div data-buyer-product-detail className="lg:grid lg:grid-cols-[minmax(0,1fr)_minmax(380px,480px)] lg:gap-8 lg:p-8">
      <div className="min-w-0"><section data-buyer-product-gallery className="relative aspect-square bg-slate-100 lg:overflow-hidden lg:rounded-lg">{image ? <Image src={image} alt={product.name} fill priority unoptimized className="object-contain" /> : <div className="grid h-full place-items-center bg-orange-50"><Store size={56} className="text-orange-200" /></div>}</section>
      {product.images.length > 1 && <div className="flex gap-2 overflow-x-auto px-4 py-3 lg:px-0">{product.images.map((item) => <button key={item.id} onClick={() => setImage(item.url)} className={`shrink-0 overflow-hidden rounded-lg border-2 bg-slate-100 ${image === item.url ? "border-orange-500" : "border-transparent"}`}><Image src={item.url} alt="商品缩略图" width={64} height={64} unoptimized className="h-16 w-16 object-contain" /></button>)}</div>}</div>
      <div data-buyer-product-purchase className="min-w-0 lg:rounded-lg lg:border lg:bg-white">
        <section className="border-t p-5 lg:border-t-0"><p className="text-2xl font-bold text-rose-600">{money(unitPrice)}</p><h1 className="mt-3 break-words text-xl font-bold leading-7">{product.name}</h1><p className="mt-2 break-words text-sm text-slate-500">{product.brand || product.category}</p>{merchantPreview && <p className="mt-3 rounded-xl bg-slate-100 p-3 text-sm text-slate-600">当前状态：{product.archived ? "已归档" : product.status === "active" ? "已上架" : "已下架"}</p>}{!merchantPreview && unavailable && <p className="mt-3 rounded-xl bg-red-50 p-3 text-sm font-semibold text-red-700">商品已下架，暂时无法购买</p>}</section>
        <section className="border-t p-5"><h2 className="font-bold">选择规格</h2><div className="mt-3 flex flex-wrap gap-2">{product.skus.map((item) => <button key={item.id} disabled={!merchantPreview && !purchasable} onClick={() => { setSelected(item.id); setQuantity(1); }} className={`max-w-full break-words rounded-xl border px-4 py-2 text-sm disabled:cursor-not-allowed disabled:opacity-50 ${sku?.id === item.id ? "border-orange-500 bg-orange-50 text-orange-700" : "border-slate-200"}`}>{item.specName}</button>)}</div>{sku && <><div className="mt-5 flex min-w-0 items-center justify-between gap-3">{sku.stock <= 0 ? <span className="min-w-0 text-sm font-semibold text-red-600">库存不足，联系商家</span> : <span className="min-w-0 text-sm text-slate-500">可选数量</span>}{!merchantPreview && <div className="flex shrink-0 items-center gap-3"><button disabled={!purchasable} onClick={() => setQuantity(Math.max(1, quantity - 1))} className="grid h-9 w-9 place-items-center rounded-lg bg-slate-100 disabled:opacity-50"><Minus size={15} /></button><b>{quantity}</b><button disabled={!purchasable} onClick={() => setQuantity(Math.min(sku.stock, quantity + 1))} className="grid h-9 w-9 place-items-center rounded-lg bg-orange-500 text-white disabled:bg-slate-300"><Plus size={15} /></button></div>}</div>{sku.tiers.length > 0 && <div className="mt-4 rounded-xl bg-orange-50 p-3 text-sm"><b>批发阶梯价</b>{sku.tiers.map((tier) => <p key={tier.minQty} className="mt-2 flex min-w-0 justify-between gap-3 text-slate-600"><span className="min-w-0">{tier.minQty}{tier.maxQty ? `-${tier.maxQty} 件` : " 件以上"}</span><b className="shrink-0">{money(tier.unitPrice)}/件</b></p>)}</div>}</>}</section>
        {product.reviews && <section className="border-t p-5"><div className="flex items-center justify-between"><h2 className="font-bold">商品评价</h2><span className="text-sm font-semibold text-amber-600">★ {product.reviews.average.toFixed(1)} · {product.reviews.count} 条</span></div><div className="mt-3 space-y-3">{product.reviews.reviews.slice(0,3).map((review,index)=><div key={index} className="rounded-xl bg-slate-50 p-3 text-sm"><div className="flex justify-between text-xs text-slate-500"><span>{review.buyerName}</span><span className="text-amber-500">{'★'.repeat(review.rating)}</span></div><p className="mt-2 text-slate-700">{review.content}</p>{review.merchantReply&&<p className="mt-2 text-orange-700">商户回复：{review.merchantReply}</p>}</div>)}</div></section>}{product.description && <section className="border-t p-5"><h2 className="font-bold">商品说明</h2><FormattedDescription text={product.description} className="mt-3 break-words text-sm leading-6 text-slate-600" /></section>}
      </div>
    </div>
    {!merchantPreview && <><ProductGroup title="为你推荐" products={recommendations} /><ProductGroup title="相关商品" products={related} /></>}
  </div>{!merchantPreview && <div className="fixed inset-x-0 bottom-0 z-30 mx-auto flex max-w-[520px] min-w-0 items-center gap-3 overflow-hidden border-t bg-white p-3 pb-[max(.75rem,env(safe-area-inset-bottom))] lg:sticky lg:bottom-6 lg:max-w-[1280px] lg:rounded-lg lg:border lg:px-8 lg:py-3 lg:shadow-lg">{message && <span className={`min-w-0 flex-1 truncate text-xs ${message === "已加入购物车" ? "text-emerald-600" : "text-red-600"}`}>{message}</span>}<button onClick={add} disabled={busy||!sku||sku.stock<1||!purchasable} className="ml-auto flex min-w-0 max-w-full shrink items-center justify-center gap-2 rounded-xl bg-orange-500 px-4 py-3 font-semibold text-white disabled:bg-slate-300"><ShoppingCart size={18} className="shrink-0" /><span className="truncate">{!purchasable ? "已下架" : sku?.stock ? busy ? "加入中…" : "加入购物车" : "暂时缺货"}</span></button></div>}</main>;
}
