...
当前版本保留支付能力接口和状态机，但未配置完整商户资料时不会发起真实扣款。新商户优先使用微信支付公钥验签，不需要把平台证书作为必填项。
...
请将以下值只写入生产服务器的受控环境文件，不要写入源码、日志、测试数据或 GitHub：
    ...
    WECHAT_PAY_PUBLIC_KEY_FILE=/run/secrets/wechat-pay-public-key.pem
    WECHAT_PAY_PUBLIC_KEY_ID=微信支付公钥ID
    WECHAT_PAY_NOTIFY_URL=https://www.kunshanjiawang.cn/api/payments/wechat/notify

其中，私钥只能来自商户平台下载的商户API证书私钥；公钥文件应来自商户平台的微信支付公钥，公钥 ID 在商户平台的公钥管理页面查看。两个文件都放在服务器受控 secrets 目录，并以只读方式挂载到容器。

平台证书是旧式兼容方案。当前代码的支付通知和退款通知验签使用微信支付公钥配置；不要只填写 `WECHAT_PAY_PLATFORM_CERT_FILE`，它不会让当前实现进入 ready 状态。

还需要在微信商户平台完成：小程序 AppID 与商户号绑定、API v3 密钥设置、商户证书配置、支付回调地址备案、退款权限开通。配置完成后再进行真实下单、回调验签、退款和对账联调。
