...
const required = [
  ...
  'pages/cart/cart.js', 'pages/messages/messages.js', 'pages/address/address.js', 'pages/orders/orders.js', 'pages/profile/profile.js', 'pages/points/points.js'
];
...
if (!app.pages.includes('pages/profile/profile')) throw new Error('profile page must be registered');
if (!app.pages.includes('pages/points/points')) throw new Error('points page must be registered');
const sitemap = JSON.parse(fs.readFileSync(path.join(root, 'miniprogram/sitemap.json'), 'utf8'));
if (!Array.isArray(sitemap.rules) || sitemap.rules.length === 0) throw new Error('sitemap.rules must contain at least one rule for WeChat upload');
if (!sitemap.rules.some((rule) => rule && rule.action === 'allow' && rule.page === '*')) throw new Error('sitemap must allow the app pages');
...
for (const marker of ['.login-button { margin-top: 40rpx; height: 88rpx; font-size: 30rpx;', '.courier-entry { position: absolute; right: 28rpx; bottom: 42rpx;', '.error { margin-top: 20rpx; padding: 18rpx; border-radius: 10rpx; background: #fff0ed; color: #c43d2e; font-size: 26rpx;']) if (!loginStyles.includes(marker)) throw new Error('login screen typography contract missing: ' + marker);
...
for (const marker of ['/api/auth/me', '/api/chat/messages', '/api/customers/profile', 'clientMessageId', 'setInterval', 'clearInterval', 'wx.downloadFile', 'previewImage']) if (!messagesPage.includes(marker)) throw new Error('messages page contract missing ' + marker);
for (const marker of ['scroll-into-view', 'bindconfirm="sendMessage"', 'message-row', 'openOrders', 'camera-icon', 'bindtap="chooseImage"']) if (!messagesView.includes(marker)) throw new Error('messages view contract missing ' + marker);
for (const marker of ['message-image', 'localImageUrl', 'bindtap="previewImage"']) if (!messagesView.includes(marker)) throw new Error('messages image rendering contract missing ' + marker);
for (const marker of ['.camera-icon {', '.camera-lens {']) if (!messagesStyles.includes(marker)) throw new Error('message camera control must use the native view structure: ' + marker);
...
for (const page of ['home', 'cart', 'orders', 'messages']) {
  const script = fs.readFileSync(path.join(root, 'miniprogram/pages', page, `${page}.js`), 'utf8');
  const view = fs.readFileSync(path.join(root, 'miniprogram/pages', page, `${page}.wxml`), 'utf8');
  if (!script.includes('openProfile')) throw new Error(`${page} must expose openProfile navigation`);
  if (!view.includes('bindtap="openProfile"')) throw new Error(`${page} bottom nav must open profile page`);
  if (!script.includes('/api/customers/profile')) throw new Error(`${page} must enforce profile completion before access`);
}
