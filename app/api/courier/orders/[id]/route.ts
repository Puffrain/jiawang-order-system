...
export async function PATCH(request:Request,{params}:{params:Promise<{id:string}>}){
  ...
  else if(action==='deliver')result=db.transaction(()=>{const updated=db.prepare("UPDATE orders SET fulfillment_status='delivered',status='closed',delivered_at=CURRENT_TIMESTAMP,delivery_proof_json=?,updated_at=CURRENT_TIMESTAMP WHERE id=? AND courier_user_id=? AND fulfillment_status='out_for_delivery'").run(proof,id,auth.session!.userId);if(updated.changes===1)completeOrderPoints(db,id);return updated;})();
