      ...
      <div className="mobile-safe-screen mx-auto w-full max-w-[460px] overflow-hidden bg-[#f7f8fb] shadow-2xl sm:min-h-[860px] sm:rounded-[32px] sm:border-[7px] sm:border-slate-900">
        <header className="safe-top sticky top-0 z-20 flex min-w-0 items-center gap-3 border-b bg-white px-4 py-3">
          <Link
            aria-label="返回买家中心"
            href="/buyer"
            className="mobile-action grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-slate-100 text-slate-700"
          >
            <ArrowLeft size={19} />
          </Link>
        ...
        <div className="space-y-4 p-4 pb-[max(2rem,env(safe-area-inset-bottom))]">
          <section className="rounded-3xl bg-orange-500 p-5 text-white">
            <div className="flex items-center gap-2 text-sm text-orange-300">
              <CircleDollarSign size={17} />
              可用积分
            </div>
            <p className="mt-3 break-words text-4xl font-bold">
              {summary ? summary.balancePoints : "--"}
            </p>
            <div className="mt-5 grid grid-cols-2 gap-3 border-t border-white/10 pt-4 text-sm">
              <div>
                ...
                <p className="mt-1 font-semibold">
                  {summary?.earnedPoints ?? "--"} 积分
                </p>
              </div>
              <div>
                <p className="text-xs text-slate-400">累计抵扣</p>
                <p className="mt-1 font-semibold">
                  {summary ? yuanFromFen(summary.redeemedFen) : "--"}
                </p>
              </div>
            </div>
          </section>
          ...
          <section className="rounded-2xl bg-white p-4 shadow-sm">
            <div className="flex items-center gap-2 font-semibold text-slate-900">
              <Gift className="text-orange-500" size={19} />
              积分规则
            </div>
            <div className="mt-3 grid grid-cols-2 gap-2 text-center text-sm">
              <div className="rounded-xl bg-emerald-50 p-3">
                <p className="text-xs text-emerald-700">完成订单</p>
                <p className="mt-1 font-bold text-emerald-800">
                  {summary ? yuanFromFen(summary.earnAmountFen) : "¥1.00"} = 1
                  积分
                </p>
              </div>
              <div className="rounded-xl bg-orange-50 p-3">
                <p className="text-xs text-orange-700">下单抵扣</p>
                <p className="mt-1 font-bold text-orange-800">
                  1 积分 ={" "}
                  {summary ? yuanFromFen(summary.pointValueFen) : "¥0.10"}
                </p>
              </div>
            </div>
            <p className="mt-3 flex items-start gap-2 text-xs leading-5 text-slate-500">
              <ShieldCheck
                ...
                size={15}
              />
              订单完成后积分自动到账；订单调整或关闭时，未使用的积分会自动退回。
            </p>
          </section>
          ...
          <section>
            ...
            <div className="space-y-2">
              {summary?.entries.map((entry) => (
                <article
                  key={entry.id}
                  className="flex min-w-0 items-center gap-3 rounded-2xl bg-white p-4 shadow-sm"
                >
                  <span
                    className={`grid h-10 w-10 shrink-0 place-items-center rounded-xl ${entry.pointsDelta >= 0 ? "bg-emerald-50 text-emerald-600" : "bg-orange-50 text-orange-600"}`}
                  >
                    <CircleDollarSign size={20} />
                  </span>
                  <div className="min-w-0 flex-1">
                    <p className="truncate text-sm font-semibold">
                      {eventLabel[entry.eventType] || "积分变动"}
                    </p>
                    <p className="mt-1 truncate text-xs text-slate-400">
                      {entry.orderNo ? `订单 ${entry.orderNo} · ` : ""}
                      {new Date(entry.createdAt).toLocaleString("zh-CN")}
                    </p>
                  </div>
                  <div className="shrink-0 text-right">
                    <p
                      className={`font-bold ${entry.pointsDelta >= 0 ? "text-emerald-600" : "text-orange-600"}`}
                    >
                      {entry.pointsDelta > 0 ? "+" : ""}
                      {entry.pointsDelta}
                    </p>
