...
发布结果：功能提交 `f9be74377a54d77659f62965368acbed082120ac` 已推送到 `release/v1.5.0-mini-courier`，不可变标签 `v1.6.1` 指向该提交。微信开发者工具使用正式 AppID 上传版本 `1.6.1`，上传包 232863 字节；这是平台开发版本上传，不等于审核通过或正式发布。服务器继续使用订单镜像 `sha256:42a2e9e4d462a72bd97fb831a7108548eff82e55866186b3efe937e7bbf19af0`，恢复点 `/root/jiawang-backups/20260906-091016-mini-bearer-190f23e` 校验通过，终检输出 `FINALIZE_PASS`；两库 `quickCheck=ok`、图片 50/50、媒体 pending/failed 0、同步 pending/dead 0。

## 2026-09-05 跨端视觉系统候选交接

本轮按用户要求只做前端视觉与布局，不触碰接口字段、权限规则、路由、数据结构、下单流程或库存服务端校验。新增 `docs/design-system.md`，定义佳旺橙色 `#E9681A`、按下色 `#C95312`、浅橙背景 `#FFF3E9`、页面背景 `#F4F6F8`、白色内容区、深色文字、成功/提示/错误语义色，以及跨端字号、间距、圆角、边框、阴影、按钮、表单、空态、加载态、错误态和禁用态规则。

Web 端已有 `app/globals.css` 和买家/后台组件继续使用统一 token；买家商品页保留搜索、分类和商品独立滚动，销量排序显示为“销量优先”，买家不显示具体库存数量，缺货显示“库存不足，联系商家”。小程序新增/统一 `miniprogram/app.wxss` token，客户登录和配送员登录顶部大图改为“佳旺”品牌标识；首页通知、搜索、分类/商品滚动、缺货提示、底栏，商品详情规格/步进器/固定底部操作栏，评价星级，配送员订单状态均完成视觉收敛。业务绑定和事件名保持不变。

本地验证：`pnpm run typecheck` PASS；`pnpm run lint` PASS（0 error，4 条既有 warning）；`pnpm run test:miniprogram-contract` PASS；全部小程序 JavaScript `node --check` PASS；`pnpm run scan:secrets` PASS；`pnpm run test:deployment-config` PASS；`pnpm exec next build --webpack` PASS；`git diff --check` PASS。

待验收边界：需要用户在微信开发者工具点击“编译”后逐页查看移动端视觉，尤其商品图片是否仍返回 401、底部导航/详情操作栏是否避开安全区、长商品名和消息内容是否换行正常。真实微信登录、图片鉴权、消息双向同步、支付和退款仍未在本轮验证。当前未部署、未提交、未推送 GitHub；`miniprogram/project.config.json` 与 `miniprogram/project.private.config.json` 是用户本地配置，继续排除。

## 2026-09-01 v1.5.2 后台滚动与搜索发布交接

用户确认在现有 Web 系统补齐滚动和搜索并上线。发布提交为 5736d8112bc47251de3e335fc4455647e564bce5，已推送至既有分支 release/v1.5.0-mini-courier，标签 v1.5.2 指向该功能提交。后续正式小程序 AppID 已确认为 wx755ca031b1ed8a25；该值仅用于本地开发者工具配置和受控发布副本，服务端仍需配置对应的 WECHAT_MINI_SECRET。

Web 买家商品页原有商品搜索、分类栏和商品列表纵向滚动保持不变；买家聊天消息记录继续独立滚动且输入区固定。后台订单管理新增按订单号、客户名、手机号搜索，订单列表增加受视口高度约束的纵向滚动和固定表头。后台客户消息新增按客户、店铺、手机号、最近消息搜索；左侧会话列表和右侧消息记录分别滚动，右侧标题和底部发送区固定。移动端继续使用单栏切换。

本地验收通过：后台布局、聊天跳转、批量触达、买家响应式契约，TypeScript，改动文件 ESLint，部署配置，数据库并发启动，敏感信息扫描，差异空白检查，以及 Webpack 生产构建（69 个页面）。隔离浏览器预览确认订单和消息搜索框已渲染，左侧会话列表计算高度 453px 且 overflow-y:auto，右侧聊天工作区高度 518px 且 overflow:hidden。旧 chat-backend-contract.mjs 仍要求已经不存在的 hadFirstActivatedAt 内部变量，属于既有过期断言，不是本次回归。

服务器候选包 SHA-256 为 81f067483c8df93e609132e13478959284e86d5fcdee138e486ddaa8de7743ae，订单镜像为 sha256:ad37b2a72bbcabb24be9cc4751887cd027859bca859197887c4879e0b993af37。恢复点 /root/jiawang-backups/20260901-192314-v1.5.2-5736d81 已逐项校验。生产终检输出 FINALIZE_PASS；公网订单、仓库健康接口及后台登录页均为 200。两库 quick_check=ok；订单商品 23、有效商品 22、图片 50、缺失 0；仓库已发布商品 22、资源 50；媒体和同步队列为 0；四个核心服务重启次数均为 0。
