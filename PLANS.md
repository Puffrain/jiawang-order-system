...
3. `completed` 首轮独立 reviewer 与 acceptance；结论分别为 REVIEW FAIL 和 ACCEPTANCE BLOCKED。
4. `in_progress` 修复迁移后故障处理、终检阻断、订单非 root 和 CI 覆盖。
5. `in_progress` 新增中英文 README，统一项目状态和发布文档。
6. `pending` 本地验证、PR、GitHub CI 和 reviewer 复审。
7. `pending` 独立恢复演练与 acceptance 复审。
